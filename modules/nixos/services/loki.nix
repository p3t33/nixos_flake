...
{ config, lib, ... }:
let
  cfg = config.customOptions.enableModule.loki;
  loki_path_prefix = "/var/lib/loki";
  log_perioud = "24h";
in
{
  options.customOptions.enableModule.loki = lib.mkEnableOption "Enable Loki log aggregation system with filesystem storage";
