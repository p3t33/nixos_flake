    ...
    services.prometheus = {
      ...
      scrapeConfigs = [ ]
        ...
        ++ lib.optionals (config.services.sonarr.enable && config.services.prometheus.exporters.blackbox.enable) [
          {
            ...
            { target_label = "__address__"; replacement = "${config.custom.shared.localHostIPv4}:${builtins.toString config.services.prometheus.exporters.blackbox.port}"; }
            ];
          }
        ]
        # sonarr metrics.
        ++ lib.optionals config.services.prometheus.exporters.exportarr-sonarr.enable [
          {
            job_name = "sonarr";
            static_configs = [{ targets = [ "${config.custom.shared.localHostIPv4}:${builtins.toString config.services.prometheus.exporters.exportarr-sonarr.port}" ]; }];
          }
        ]
        ++ lib.optionals config.services.prometheus.exporters.node.enable [
          {
            job_name = "node_exporter";
            static_configs = [{ targets = [ "${config.custom.shared.localHostIPv4}:${builtins.toString config.services.prometheus.exporters.node.port}" ]; }];
          }
        ]
        ++ lib.optionals config.services.restic.server.enable [
          {
            ...
            static_configs = [{ targets = [ "${config.custom.shared.localHostIPv4}:9005" ]; }];
          }
        ];
    };
  };
