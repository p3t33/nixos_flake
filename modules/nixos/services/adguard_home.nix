...
let
  cfg = config.customOptions.enableModule."adguardHome";
  dnsPort = 53;
in
{
  options.customOptions.enableModule."adguardHome" = lib.mkEnableOption "Enable AdGuard Home service";
            ...
            ++ lib.optionals config.services.deluge.enable [
              {
                ...
                answer = "${config.customGlobalOptions.${hostSpecific.hostName}.ip}";
              }
            ]
