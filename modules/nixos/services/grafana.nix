  ...
  config = lib.mkIf config.services.grafana.enable {
    ...
    services.grafana = {
      settings = {
          ...
          server = {
            http_addr = "${config.customGlobal.anyIPv4}";
            http_port = 3001;
         };
      };
          ...
          ++ lib.optionals config.services.loki.enable [
            {
              name = "Loki";
              type = "loki";
              url = "http://${config.customGlobal.localHostIPv4}:${builtins.toString config.services.loki.configuration.server.http_listen_port}";
              access = "proxy";
              jsonData = {
                maxLines = 1000;
              };
