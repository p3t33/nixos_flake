...
let
  cfg = config.customOptions.enableModule.kvm;
in
{
  options.customOptions.enableModule.kvm = lib.mkEnableOption "Enable KVM/QEMU virtualization environment";
  ...
  config = lib.mkIf cfg {
    ...
    systemd.services.libvirt-enable-default-network = {
      ...
      after = [ config.systemd.services.libvirtd.name ];
