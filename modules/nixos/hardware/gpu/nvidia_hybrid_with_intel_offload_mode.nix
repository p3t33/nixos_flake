{ config, lib, ... }:
let
  cfg = config.customOptions.enableModule.nvidiaHybridWithIntelOffLoadMode;
in
{
  options = {
    customOptions = {
      enableModule.nvidiaHybridWithIntelOffLoadMode = lib.mkEnableOption "Enable NVIDIA PRIME offload (hybrid with Intel)";
