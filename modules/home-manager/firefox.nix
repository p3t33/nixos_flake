...
let
  cfg = config.customOptions.enableModule.firefox;
in
{
  options.customOptions.enableModule.firefox = lib.mkEnableOption "Enable Firefox with custom policies and extensions";
  config = lib.mkIf cfg {
    programs.firefox = {
      enable = true;
      policies = {
