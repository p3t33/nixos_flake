  ...
  config = lib.mkIf config.programs.tmux.enable {
    programs.tmux = {
      ...
      plugins = with pkgs.tmuxPlugins; [
        ...
        # Copy inside tmux into system clipboard
        # ======================================
        #
        # OSC 52
        # ------
        # Allows a program running inside a terminal emulator to ask the terminal to pass
        # the copy  text to the to system clipboard.
        #
        # a note about neovim(as it often runs inside tmux)
        # -------------------------------------------------
        # unlike tmux neovim has a dynamic way to determine what +y clipboard to use
        # based on priority. A configured override takes priority.
        # It will pick one based on if it runs(those are some of the options):
        # - locally: will use system clipboard directly if display server is available,
        #   and if not will ask the terminal if it supports OSC 52 and use that.
        # - locally inside tmux: will use tmuxs paste buffer.
        # - over ssh: Assuming this is a server, will ask the terminal emulator if it supports
        #   OSC 52 and use that.
        # - over ssh inside of tmux: will use tmuxs paste buffer.
        #
        # how tmux works with OSC 52
        # --------------------------
        #
        # for starters tmux uses terminfo(based on $TERM), to figure out the terminal
        # capabilities, one of which being how to use OSC 52 with the terminal.
        #
        # At which point copy actions inside tmux be it from native tmux yank, tmux plugins
        # or neovim running inside of tmux can be done in such a way that they will be
        # passed to tmux, which will pass them via OSC 52 to the terminal emulator it tuns
        # in and form their the terminal will copy them into its system cliborad.
        # *neovim will "just work" while tmux yank and plugins will need to use -w
        # modifier.

        # tmux has its own buffer it copies text into, and when setting:
        # set-clipboard external(which is the default for tmux v3.6a), it allows t
        # tmux to send clipboard writes over OSC 52 to cable terminals, and
        # the terminal to copy it into the system clipboard.

        # why not to use system cliborad directly on a system with display server(xorg/wayland)?
        # -------------------------------------------------------------------------------------
        # tmux can be configured to use system cliborad directly assuming is has a
        # display server by configuring its copy command to pipe text into xclip/wl-copy.
        #
        # This method will break when used on a server which has no display server and in a
        # way goes against the use OSC 52 which tmux supports and which works perfectly
        # fine be it on a machine with or without a display server.
        #
        ...
        # used for:
        # - command-line copy
        # - copy the current pane’s working directory(PWD).
        {
          plugin = yank;
          extraConfig = ''
            set -g @override_copy_command '${lib.getExe' pkgs.coreutils "cat"} > /dev/null'
          '';
        }
        # used for:
        # A recognized item on the visible pane—path, URL, hash, IP, etc(and then by pressing a letter it is copied)
        # it is a quick and easy method to copy specific string.
        #
        # uses tmux set-buffer -w, putting it into tmux buffer and asking tmux to send it to the terminal cliborad
        # via OSC 52.
        {
          plugin = tmux-thumbs;
          extraConfig = ''
        ...
        # used for:
        # is used as a fuzzy finder inside of a pane, which can be copied, filtered...
        # Also uses OSC 52.
        {
          plugin = extrakto;
          extraConfig = ''
        ...
        # Native tmux copy bindings
        # -------------------------
        # are part of the general extraConfig section and use the same OSC 52 principle.
        # they are based on prefix+[, select with v/V/Ctrl-v, press y.
      ...
      extraConfig = ''
        ...
        # Pin the clipboard policy instead of relying on tmux's default: send
        # tmux copies via OSC 52, but reject raw OSC 52 clipboard writes from panes.
        ...
        bind-key Y run-shell '${tmux} display-message -p -t "#{pane_id}" "##{pane_current_path}" | ${lib.getExe pkgs.perl} -pe "chomp if eof" | ${tmux} load-buffer -w -t "#{client_name}" -'
        ...
        # not sure if it might break some flow and it requires perl.
        # so I left the version that adds \n and has no dependencies in comment.
        #bind-key -T copy-mode-vi y send-keys -X copy-selection-and-cancel
        bind-key -T copy-mode-vi y send-keys -X copy-pipe-and-cancel "${lib.getExe pkgs.perl} -pe 'chomp if eof' | ${tmux} load-buffer -w -"
        # ==================================
