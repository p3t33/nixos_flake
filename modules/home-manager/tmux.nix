  ...
  config = lib.mkIf config.programs.tmux.enable {
    programs.tmux = {
      ...
      plugins = with pkgs.tmuxPlugins; [
        ...
        # is set. But most of the termenals need to use an external clipboard provider.
        ...
        # - yank the pwd.
        {
          plugin = yank;
          extraConfig = ''
            set -g @override_copy_command '${wlCopy}'
          '';
        }
        ...
        # clipboard as swell.
        {
          plugin = tmux-thumbs;
          extraConfig = ''
            set -g @thumbs-command '${lib.getExe pkgs.tmux} set-buffer -- "{}" && ${lib.getExe pkgs.tmux} display-message "Copied {}" && printf %s "{}" | ${wlCopy}'
        ...
        }
        {
          ...
          extraConfig = ''
            ...
            set -g @extrakto_clip_tool '${wlCopy}'
          '';
        }
      ...
      extraConfig = ''
        ...
        # plugins use an external provider so not to depend on the graces of the terminal.
        ...
        bind-key -T copy-mode-vi y send-keys -X copy-pipe-and-cancel "${lib.getExe pkgs.perl} -pe 'chomp if eof' | ${lib.getExe pkgs.tmux} load-buffer - ; ${lib.getExe pkgs.tmux} save-buffer - | ${wlCopy}"
        # ==================================
